            ...
            <div id="con_one_1">
                <div id="1">
                    ...
                    <form action="userInfo" method="post">
                        ...
                        <td width="352"><input type="text" name="useridnumber" style="width:200;height:40" size="30" /></td>
                        </tr>
                    ...
                    <td>
                        <p>
                            <label>
                                <input type="radio" name="usergender" value="男" id="RadioGroup1_0" />
                                男</label>

                            <label>
                                <input type="radio" name="usergender" value="女" id="RadioGroup1_1" />
                                女</label>
                            <br>
                        </p></td>
                    </tr>
            ...
            <div id="con_one_3" style="display:none;">
                ...
                <div>
                    <form action="demo-form.php">
                        选择图片:
                        <br>     <input type="file" name="img" multiple>
                        <br>
                        <input type="submit">
                    </form>
                </div>
...
<!-- //bootstrap working-->
    ...
    <%--<form action="updateUser" method="post">--%>
        <%--欢迎来到用户后台，${user.username}--%>
        <%--账号ID：<input type="text" name="id" value="${user.id}" disabled="disabled"/><br>--%>
        <%--用户名：<input type="text" name="username" value="${user.username}" disabled="disabled" style="background-color: #cccccc"><br>--%>
        <%--密码：<input type="password" name="password" value="${user.password}"><br>--%>
        <%--手机号：<input type="text" name="userphone" value="${user.userphone}" placeholder="暂无"><br>--%>
        <%--性别：<input type="text" name="usergender" value="${user.usergender}" placeholder="暂无"><br>--%>
        <%--身份证号：<input type="text" name="useridnumber" value="${user.useridnumber}" placeholder="暂无"><br>--%>
        <%--地区：<input type="text" name="userarea" value="${user.userarea}" placeholder="暂无"><br>--%>
        <%--账号创建时间：<input type="text" name="usercreatetime" value="<fmt:formatDate value="${user.usercreatetime}" type="date" dateStyle="default"/>" disabled="disabled"><br>--%>
        <%--<div style="color: red">${requestScope.message}</div>--%>
        <%--<input type="submit" value="提交">--%>

        <%--&lt;%&ndash;账号ID：${user.id}&ndash;%&gt;--%>
        <%--用户名：${user.username}<br>--%>
        <%--&lt;%&ndash;密码：${user.password}&ndash;%&gt;--%>
        <%--手机号：${user.userphone}<br>--%>
        <%--性别：${user.usergender}<br>--%>
        <%--身份证号：${user.useridnumber}<br>--%>
        <%--地区：${user.userarea}<br>--%>
        <%--账号创建时间：<fmt:formatDate value="${user.usercreatetime}" type="date" dateStyle="default"/>--%>
        <%--&lt;%&ndash;<div style="color: red">${requestScope.message}</div>&ndash;%&gt;--%>
        <%--&lt;%&ndash;<input type="submit" value="提交">&ndash;%&gt;--%>

        <%--&lt;%&ndash;账号ID：<input type="text" name="id" value="${user.id}" disabled="disabled"/><br>&ndash;%&gt;--%>
        <%--用户名：${sessionScope.user.username}<br>--%>
        <%--&lt;%&ndash;密码：<input type="password" name="password" value="${user.password}"><br>&ndash;%&gt;--%>
        <%--手机号：<input type="text" name="userphone" value="${user.userphone}" placeholder="暂无"><br>--%>
        <%--性别：<input type="text" name="usergender" value="${user.usergender}" placeholder="暂无"><br>--%>
        <%--身份证号：<input type="text" name="useridnumber" value="${user.useridnumber}" placeholder="暂无"><br>--%>
        <%--地区：<input type="text" name="userarea" value="${user.userarea}" placeholder="暂无"><br>--%>
        <%--&lt;%&ndash;账号创建时间：<fmt:formatDate value="${user.usercreatetime}" type="date" dateStyle="default"/><br>&ndash;%&gt;--%>
        <%--<div style="color: red">${requestScope.message}</div>--%>
        <%--<input type="submit" value="提交">--%>
    </form>
</body>
